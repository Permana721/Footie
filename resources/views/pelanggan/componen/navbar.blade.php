    ...
    @auth
        @if(Auth::user()->role == 1)
            ...
            <form action="{{ url('/search') }}" class="navbar-form">
                <input type="search" placeholder="Cari di LahAda" class="navbar-form-search" name="search" value="{{ Request::get('search') }}" aria-label="search" id="search-input" autocomplete="off">
